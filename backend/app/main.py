...
app=FastAPI(title="StockPulse India API",version="4.6.1")
...
_session=requests.Session();_session.headers.update(HEADERS);_cache:dict[str,tuple[float,Any]]={}
...
def _rows_to_universe(rows):
    out=[]
    for x in rows or []:
        s=str(x.get("symbol") or "").strip().upper();p=num(x.get("lastPrice"))
...
@app.get("/health")
def health():
    universe,live=current_universe();return {"status":"ok","service":"stockpulse-india-api","version":"4.6.1","data_source":"NSE live" if live else "fallback","universe_size":len(universe)}
...
def market():
    ...
    for x in rows:
        ...
        if name in {"NIFTY 50","NIFTY BANK"} or x.get("indexSymbol") in {"NIFTY","NIFTY BANK"}:result.append({"symbol":"NIFTY 50" if name=="NIFTY 50" else name,"price":num(x.get("last")),"change_pct":num(x.get("percentChange"))})
    ...
    sensex=next((x for x in rows if str(x.get("index") or "").upper()=="SENSEX"),None);result.append({"symbol":"SENSEX","price":num(sensex.get("last")) if sensex else None,"change_pct":num(sensex.get("percentChange")) if sensex else None})
