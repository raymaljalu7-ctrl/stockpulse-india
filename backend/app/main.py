...
app = FastAPI(title="StockPulse India API", version="4.3")
...
HEADERS={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/139 Safari/537.36","Accept":"application/json,text/plain,*/*","Referer":"https://www.nseindia.com/"}
_session=requests.Session(); _session.headers.update(HEADERS)
_cache: dict[str, tuple[float,Any]]={}

# Resilient seed universe. It is deliberately used only when NSE blocks/rate-limits the server.
# The UI always labels this as fallback data; once NSE responds normally, live values take over.
BASE_PRICES={
"RELIANCE":1390,"TCS":3120,"HDFCBANK":965,"ICICIBANK":1410,"INFY":1510,"BHARTIARTL":1780,"ITC":410,"LT":3810,"SBIN":790,"AXISBANK":1210,
"KOTAKBANK":2010,"BAJFINANCE":1000,"BAJAJFINSV":2010,"MARUTI":15400,"M&M":3450,"TATAMOTORS":690,"TATASTEEL":165,"JSWSTEEL":1090,"TITAN":3850,
"SUNPHARMA":1770,"HINDUNILVR":2650,"HCLTECH":1490,"TECHM":1600,"WIPRO":520,"NTPC":420,"POWERGRID":360,"ONGC":245,"COALINDIA":410,
"BEL":395,"ADANIENT":2500,"ADANIPORTS":1450,"TRENT":5100,"ETERNAL":310,"TATACONSUM":1100,"ULTRACEMCO":12400,"GRASIM":2900,"NESTLEIND":1150,
"ASIANPAINT":2450,"BRITANNIA":5700,"CIPLA":1530,"DIVISLAB":6200,"DRREDDY":1260,"APOLLOHOSP":7800,"EICHERMOT":6400,"HEROMOTOCO":5900,
...
def fallback_universe():
    ...
    for i,s in enumerate(NIFTY50):
        p=BASE_PRICES.get(s,500.0)
        # deterministic pseudo-market movement; this is not presented as live data
        h=hashlib.sha256(s.encode()).digest()
        r30=((h[0]/255)-.5)*24
        r365=((h[1]/255)-.35)*70
        ch=((h[2]/255)-.5)*4
        pos=max(5,min(98,50+r365*.45))
...
def snapshot(q:dict, live:bool):
    p=q["price"]; r30=q["perChange30d"]; r365=q["perChange365d"]; pos=max(0,min(100,(p-q["yearLow"])/(q["yearHigh"]-q["yearLow"])*100 if q["yearHigh"]>q["yearLow"] else 50))
...
def stock(symbol:str):
    ...
    x=snapshot(q,live); x.update({"company_name":q["companyName"],"sector":q["industry"],"industry":q["industry"],"business_summary":"Quantitative StockPulse market snapshot.","52w_high":q["yearHigh"],"52w_low":q["yearLow"],"pros":x["why"][:5],"cons":["Fundamental ratios are not available in the current public-data fallback"]})
    return x
...
def market():
    ...
    if not result:
        result=[{"symbol":"NIFTY 50","price":25000,"change_pct":0},{"symbol":"NIFTY BANK","price":55000,"change_pct":0}]
