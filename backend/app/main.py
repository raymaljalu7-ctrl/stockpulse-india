...
def live_universe():
    ...
    alt=load_universe()
    if len(alt)>=300:return alt
    names=["NIFTY 500","NIFTY 200","NIFTY 100","NIFTY NEXT 50","NIFTY MIDCAP 150","NIFTY SMALLCAP 250"]
    merged={x["symbol"]:x for x in out}
    for name in names:
        d=nse_get("/api/equity-stockIndices",{"index":name},30);rows=_rows_to_universe((d or {}).get("data",[]) if isinstance(d,dict) else [])
        for x in rows:merged[x["symbol"]]=x
        if len(merged)>=300:break
