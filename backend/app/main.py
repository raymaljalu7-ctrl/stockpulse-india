...
app = FastAPI(title="StockPulse India API", version="4.6.0")
...
def nse_get(path:str,params:dict[str,Any]|None=None,ttl:int=30):
    ...
    try:
        r=_session.get("https://www.nseindia.com"+path,params=params,timeout=12)
...
def live_universe():
    # First choice: NSE's all-stocks breadth feed, which is broader than NIFTY indices.
    d=nse_get("/api/equity-stock",{"index":"allstocks"},60)
    rows=(d or {}).get("data",[]) if isinstance(d,dict) else []
    out=_rows_to_universe(rows)
    if len(out)>=500: return out
    # Second choice: NIFTY 500, so the scanner still covers far more than NIFTY 50.
    d=nse_get("/api/equity-stockIndices",{"index":"NIFTY 500"},60)
    out=_rows_to_universe((d or {}).get("data",[]) if isinstance(d,dict) else [])
    if len(out)>=300: return out
    # Last live fallback: NIFTY 50.
    d=nse_get("/api/equity-stockIndices",{"index":"NIFTY 50"},60)
    return _rows_to_universe((d or {}).get("data",[]) if isinstance(d,dict) else [])
...
def snapshot(q:dict,live:bool):
    ...
    momentum=max(0,min(100,50+r30*1.5+r365*.12+q["change_pct"]*2)); technical=max(0,min(100,50+r30*1.3+r365*.08+(pos-50)*.25))
    # Short-term ranking deliberately emphasizes recent momentum, today's move and 52W structure.
    short_term=max(0,min(100,.50*momentum+.30*technical+.20*pos))
    score=max(0,min(100,.45*technical+.30*momentum+.15*pos+.10*short_term))
    ...
    upside=max(-20,min(60,.55*r30+.12*r365+.22*(short_term-50)))
    short_rec="STRONG BUY" if short_term>=82 and upside>=6 else "BUY" if short_term>=70 and upside>=3 else "WATCH" if short_term>=55 else "AVOID"
...
@app.get("/health")
def health():
    universe,live=current_universe(); return {"status":"ok","service":"stockpulse-india-api","version":"4.6.0","data_source":"NSE live" if live else "fallback","universe_size":len(universe)}

@app.get("/api/screener/top")
def top(limit:int=5000,min_quality:float=0):
    universe,live=current_universe(); items=[snapshot(q,live) for q in universe if q["price"]>0]; items=[x for x in items if x["final_rank_score"]>=min_quality]; items.sort(key=lambda x:(x["short_term_score"],x["final_rank_score"],x["estimated_upside_pct"]),reverse=True)
    return {"generated_at":datetime.now(timezone.utc).isoformat(),"items":items[:max(1,min(limit,5000))],"universe_size":len(universe),"data_source":"NSE live" if live else "Fallback market snapshot"}
...
def market():
    ...
    result=[]
    for x in rows:
        name=str(x.get("index") or x.get("indexSymbol") or "")
        if name in {"NIFTY 50","NIFTY BANK"} or x.get("indexSymbol") in {"NIFTY","NIFTY BANK"}:
            result.append({"symbol":"NIFTY 50" if name=="NIFTY 50" else name,"price":num(x.get("last")),"change_pct":num(x.get("percentChange"))})
